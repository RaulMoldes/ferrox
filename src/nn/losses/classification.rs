    ...
    T: FerroxCudaF,
{
    ...
    pub fn new(reduction: ReductionType, apply_softmax: bool, class_axis: Option<usize>) -> Self {
        Self {
            reduction,
            apply_softmax,
            class_axis,
    ...
    pub fn from_logits(reduction: ReductionType, class_axis: Option<usize>) -> Self {
        Self::new(reduction, true, class_axis)
    }
    ...
    pub fn from_probabilities(reduction: ReductionType) -> Self {
        Self::new(reduction, false, None) // If no softmax is applied class_axis is not needed
    }
}
    ...
    T: FerroxCudaF,
{
    fn forward(
        ...
        let log_probabilities = graph
            ...
            .map_err(|e| format!("CCE log computation failed: {}", e))?;

        // Step 3: Element-wise multiply with targets: targets * log(probabilities)
