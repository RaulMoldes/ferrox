...
impl<'a> CudaOps<'a> {
    ...
    where
        T: CudaOperationType<T> + From<i32>,
    {
        let one = T::from(1);
        self.full(shape, one)
    }
    ...
    where
        T: CudaOperationType<T> + Copy + 'static,
    {
    ...
    where
        T: CudaOperationType<T> + From<f32> + 'static,
    {
