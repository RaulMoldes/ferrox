...
fn benchmark_matmul(
    ...
    backend.kernels().launch_matmul(
        cfg, &a_gpu, &b_gpu, &mut c_gpu, dim as i32, dim as i32, dim as i32,
    )?;
    backend.synchronize()?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if tests_failed == 0 {
        ...
        eprintln!(
            " {} test(s) failed. Please check your CUDA setup.",
            tests_failed
        );
        std::process::exit(1);
    }
...
fn test_matrix_multiplication(backend: &CudaBackend) -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        result[0] != 0.0,
        "Matrix multiplication produced zero result"
    );
