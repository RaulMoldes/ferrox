...
fn test_large_tensor_operations() {
    ...
    match cpu_tensor.to_cuda() {
        ...
        Err(e) => {
            println!("Large tensor allocation failed (expected on low-memory GPUs): {}", e);
        }
    }
}
        ...
        match cpu_tensor.to_cuda() {
            Ok(cuda_tensor) => {
                ...
                for (original, transferred) in cpu_tensor.data.iter().zip(back_to_cpu.data.iter()) {
                    assert!((original - transferred).abs() < 1e-6, 
                           "Data corruption after transfer {}: {} vs {}", i, original, transferred);
                }
            }
            Err(e) => {
